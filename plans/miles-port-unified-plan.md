...
- ❌ **A19** — Re-promoting `_preempted_engines` to routing / dispatch / attribution / resize-safety state. **The responsibility is permanently re-scoped to `router.enabled_workers` (single source of truth).** The set may remain only as `RolloutManager`'s abort-idempotency cache, with its lifecycle (lazy init / update on abort / discard on `offloaded` transition / reset on shrink failure) confined to `_abort_engines` and a small set of private helpers colocated with it (`_release_abort_idempotency_for`, `_reset_abort_idempotency_for`). Mutations from any other RolloutManager method (or from any caller outside the manager) remain forbidden.
...
- F12 (M11.1)：shared PG 不是可选 glue。MILES 必须接受 RLix/ROLL shared PG，并显式完成
  ...
  **Driver wiring status (M11.1 known-gap)**: `examples/rlix/run_miles_rlix.py` is split
  into two iters. The current iter-16 surface establishes only the env-var guard +
  F10 startup invariant (`assert_rlix_topology`) + `cluster_device_mappings`
  derivation; it **does not** construct the orchestrator, MilesCoordinator,
  `coordinator.create_pipeline_actor.remote(...)`, `pipeline.initialize_pipeline()`,
  or the main loop. Invoking `python -m examples.rlix.run_miles_rlix` validates topology
  and exits — no GPUs are allocated and no actors are created. **M11.1+M11.2 GPU
  smoke (Gates 1, 2, 2.5, 3, 4) cannot run from this driver until the wiring iter
  lands**; the wiring iter is intentionally deferred so that iter-16 could ship the
  startup-validation surface without depending on the full RLix-side scheduler /
  coordinator / pipeline plumbing landing in a single change.
...
- M11.2 unified-plan scope (Gate 4 happy path):
  ...
  **Execution-note boundary for M11.1-only or combined M11.1+M11.2 coding runs**: an
  implementation plan may land narrow M11.2-prep interfaces only when doing so prevents
  immediate contract churn in M11.2 (for example the entry-state dispatch shape around
  `shell` / `offloaded`). A combined M11.1+M11.2 coding run may land the full M11.2
  happy-path code surface (F37 full-INIT pattern, F38 dispatch shape, F39 C20 suspend,
  F40 base v=-1, F76/F77/F78 dev-only scaffolding). Either form **does NOT** move
  Gate 4 acceptance into the coding phase. Dual-pipeline runtime behavior, 0-active
  runtime evidence, donor-shrink ordering evidence, and all Gate 4 pass/fail decisions
  remain M11.2 smoke-phase work.
...
For execution plans that use per-iteration Codex review, per-commit quality can be
gated by static checks plus review, while the GPU smoke test is run after the
complete coding set (M11.1, or combined M11.1+M11.2) lands. Such a run should be
described as code-complete until the post-coding smoke evidence exists.
...
当前 milestone **不解禁** `multi_turn.generate` 内的 partial_rollout 守卫：

- [miles/rollout/generate_hub/multi_turn.py:29](external/miles/miles/rollout/generate_hub/multi_turn.py#L29)
  `assert not args.partial_rollout, "Partial rollout is not supported"` — **保留无条件
  assert** (RLix mode 与 standalone mode 都生效). `multi_turn.generate` 从未实现
  partial-rollout resume 语义 (没有 `single_turn.py` 那样的 `len(sample.response) > 0`
  short-circuit), 之前响应会被静默重新 tokenize 成新 prompt; 该 assert 是长期存在的
  standalone 安全护栏. 在 RLix mode 下同样必须保留: radix middleware 已禁用,
  turn-level redispatch 也要求非 streaming JSON, partial_rollout 在两种模式下都不该
  生效. C29 / C17 与既有 `tests/fast/rollout/generate_hub/test_multi_turn.py::TestExitConditions::test_partial_rollout_not_supported`
  共同约束 (参见 plans/miles-port-unified-plan.debug.md REV-003).
...
方法）。** sync_base_weights_to_active 采用 **release-before-RPC 模式**: 在
`_resize_sync_lock` 内同步更新 `coordinator._cache_ready_step` + 快照
`_active_engine_indices` 和 service handle, **释放锁**, 然后在锁外执行
`service.sync_selected_workers` atomic unit (per-bucket transport + finalize fan-out +
per-engine set_weight_version). 同一模式适用于 `_expand_workers` 的 Runtime 分支
(snapshot under lock → 4 个 Ray RPC 在锁外串行 → commit `_active_engine_indices`
under lock). **锁不跨 Ray RPC 持有** — 否则 cpu_serialize sync 期间 (~150 s
asyncio.wait_for 上限) 任何并发 coordinator 调用 (progress 报告 / `get_active_engines`
/ 其它 resize) 全部排队. atomic unit 由 `_cache_ready_step` 的快照值携带
(锁外 RPC 用快照, 不再读 `self._cache_ready_step`), 期间任何并发的
`_expand_workers` 也读到的是新值. 返回时 active engines 权重已对齐, coordinator
state 已发布。GPU 释放信号表示 "我的 active engines 权重一致 + coordinator 已发布
最新 version", 不是 "训练完成". 由 `after_training` hook 序列强制。
...
Pipeline actor (after_training, step=N):
  ...
  │     └── Coordinator actor (release-before-RPC):
  │           # ----- Phase 1: snapshot under _resize_sync_lock -----
  │           acquire _resize_sync_lock
  │           self._cache_ready_step = N
  │           # 锁内发布新 version; 任何并发 _expand_workers 拿到的是新值.
  │           active_engines = frozenset(self._active_engine_indices)
  │           if not active_engines:
  │               release _resize_sync_lock
  │               return N   # 全重叠退化拓扑 (Edge case 1); version 已发布
  │           service = self._ensure_model_update_service()
  │           release _resize_sync_lock
  │           # ----- Phase 2: RPC 在锁外执行 (avoid lock-across-RPC) -----
  │           sync_id = make_sync_id(pipeline_id, step=N)
  │           ray.get(service.sync_selected_workers.remote(
  ...
  │             # 锁不持有, 因此并发的 progress 报告 / get_active_engines 不排队.
  ...
  └── notify_release_cluster_gpus(actor_train)
        ...
        # _resize_sync_lock 内更新了 coordinator state (Phase 1).
        # publish_cache_ready_step 仍保留, 仅用于 init bootstrap 发布 -1 (Pipeline B
        # 在 first after_training 前就可能被 expand). 退化拓扑下 sync 短路返回也已
        # 在 Phase 1 更新 coordinator state.

Coordinator actor (resize_infer, scheduler-driven, no pipeline callback):
  └── _expand_workers(target_engines):           ← release-before-RPC pattern
        # ----- Phase 1: snapshot under _resize_sync_lock -----
        acquire _resize_sync_lock
        ...
        service = self._ensure_model_update_service()
        release _resize_sync_lock
        # ----- Phase 2: 4 个 Ray RPC 全部在锁外串行 -----
        entry_states = ray.get(rollout_manager.get_engine_states.remote(target_engines))
            # manager 是 Ray actor, 自身串行化; 与并发 resize_infer 在 manager 层
            # 互斥. 这里读 entry_states 不需要 coordinator-side 锁.
...
- M11.1 MVP gate: Gate 1, 2, **2.5 (NCCL selective sync)**, 3 (single pipeline).
  ...
  plan 验证范围内。Implementation execution may use per-iter static checks plus
  Codex review, but Gate 1/2/2.5/3 (and Gate 4 (a)–(f) when M11.2 is included in
  the same coding pass) smoke evidence is collected after the complete coding set
  (M11.1, or combined M11.1+M11.2) lands.
...
| **M11.2** Infer-pool placement provider routing (`MilesPipeline._init_phase_b_infer` 路由 PG 通过 `self._placement_provider.get_all_rollout_engine_placements()` 而非 `_create_placement_group`) | b241af4 P1-2 pragmatic fix 让 `_init_phase_b_infer` 直接调 `_create_placement_group(rollout_num_gpus)` (standalone-MILES 路径), 绕过 rlix scheduler ledger. M11.1 single-pipeline 安全 (scheduler grant exhausts pool, Ray PG 与 rlix ledger 引用同一组 GPU); M11.2 dual-pipeline 会让 Ray oversubscribe rlix scheduler 已分给同伴 pipeline 的池. 当前用 fail-fast assert (`len(set(infer_allocated)) >= rollout_num_gpus`) 阻止静默错路, 但需要把 `WorkerPlacement` list 翻译回 RolloutManager 期望的 `(pg, reordered_bundle_indices, reordered_gpu_ids)` legacy tuple 才能正式启用. 与 M11.2 dual-pipeline smoke 里程碑同 (Gate 4 (c) — Pipeline B init under contention) |
