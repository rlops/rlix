...
All tests run without Ray or GPU â€” workers are simple Python fakes.
"""
...
# ---------------------------------------------------------------------------
# promote_base â€” calls build_latest_bucket_cache(-1) then promote(-1)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# promote â€” calls promote_active_checkpoint(version)
# ---------------------------------------------------------------------------
...
def test_promote_does_not_call_build(lifecycle, workers):
    """promote() must NOT call build_latest_bucket_cache â€” that's the pipeline's job."""
...
def test_expand_workers_sync_before_expand_sampler():
    ...
    if pipeline_mod_name not in _sys.modules:
        return  # pipeline not importable in this env â€” skip
    ...
    # Direct unit test without importing the heavy pipeline â€” just test the ordering logic.
