...
| [miles/rollout/generate_hub/multi_turn.py](../external/miles/miles/rollout/generate_hub/multi_turn.py) | Multi-turn trajectory generator | **keep** unconditional `partial_rollout` assert (no resume semantics in either mode); force `payload["stream"]=False` in RLix mode (gated on `_is_rlix_mode()`); snapshot-then-retry; `MAX_ATTEMPTS = total_engine_count`; raise `EnginePreemptedError` on exhaustion | swallow `EnginePreemptedError`; group-recycle as fallback; replay env steps on aborted turn; **gating the partial_rollout assert behind `rlix_mode` (REV-003: re-allowing partial_rollout under standalone re-tokenizes prior responses as fresh prompts)** | D3.B |
...
| examples/rlix/run_miles_rlix.py (NEW) | RLix entry driver | orchestrator + coordinator + pipeline.initialize_pipeline + main loop. **Iter-16 known-gap stub** (REV-004): current file lands env-var guard + F10 startup invariant + `cluster_device_mappings` derivation only; orchestrator / coordinator / pipeline construction + main loop intentionally deferred to a wiring iter. M11.1+M11.2 GPU smoke (Gates 1, 2, 2.5, 3, 4) cannot run from this driver until the wiring iter lands. | top-level try/except + `ray.shutdown` (raise propagate; `ray stop` CLI) | D2.C |
...
| `_preempted_engines` set is `RolloutManager._abort_engines` internal idempotency cache only (M11.2 MVP) | D4.A | A19 — re-promoting `_preempted_engines` is forbidden; responsibility permanently re-scoped to `router.enabled_workers` (architectural, not deferral); manager-local set kept only as abort-RPC idempotency guard, with its lifecycle (init / update / discard / failure-reset) confined to `_abort_engines` and its private helpers (`_release_abort_idempotency_for`, `_reset_abort_idempotency_for`) | High — re-promoting to attribution/routing state breaks A19 cascade across Feature 2 + Feature 11 + Gate 1 |
...
- A19: Re-promoting `_preempted_engines` to routing / dispatch / attribution / resize-safety state (responsibility permanently re-scoped to `router.enabled_workers`; set may remain only as `_abort_engines`-owned idempotency cache, with its lifecycle confined to `_abort_engines` plus its private helpers `_release_abort_idempotency_for` / `_reset_abort_idempotency_for`).
...
- **M11.2 follow-up (REV-006)**: routing the inference-pool placement group through `MilesPlacementProvider.get_all_rollout_engine_placements()` instead of the standalone `_create_placement_group(rollout_num_gpus)` path used in `MilesPipeline._init_phase_b_infer`. Current code uses the standalone path as an M11.1 pragmatic fix (b241af4 P1-2) and bypasses the rlix scheduler ledger; safe in single-pipeline because the scheduler grant exhausts the pool, but in M11.2 dual-pipeline Ray would happily oversubscribe a pool the rlix scheduler is already serving to a peer pipeline. A fail-fast assert (`len(set(infer_allocated)) >= rollout_num_gpus`) blocks silent mis-routing today; the proper fix translates the `WorkerPlacement` list back into the `(pg, reordered_bundle_indices, reordered_gpu_ids)` tuple `RolloutManager.__init__` consumes. Same milestone as Gate 4 (c) Pipeline B init under contention.
...
`shell` is terminal-on-the-left for shrink — never `active → shell`. M11.2 MVP attribution / routing / dispatch / resize-safety all read from `router.enabled_workers` (router admission state — single source of truth); `_preempted_engines` is a manager-local abort-RPC idempotency cache only (A19 — responsibility permanently re-scoped to `router.enabled_workers`; cache lifecycle confined to `_abort_engines` and its private helpers — see §3.1 Layer 1 in source plan).
