...
## §12 Scheduler 核心与 B-13（guide §3.5）
...
| S12-INFLATE-WINDOW | demand 膨胀不限于 bootstrap 窗口 | `planner.py:213-217` | 注释说 "Inflate demand in the bootstrap window (request sent, no progress yet)"，但条件只检查注册表 membership——entry 存活至 release/`clear_progress`，rollout 进行中（progress > 0）仍然 `remaining += step_target`，需求双计。双 pipeline 同时在 rollout 时近似对称，影响小；单侧持有 entry 时会偏斜 gap-ratio 分配。低风险。 |
