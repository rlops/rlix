...
if path == "generate":  # 仅 multi_turn /generate path 注入 metadata
    try:
        ...
        pass  # non-JSON body 不注入 (RLix mode 禁用 radix middleware, 流式由
              # multi_turn 强制 payload["stream"] = False + F10 validation 阻挡)
return {"request_body": body, "response_body": content,
        "status_code": response.status_code, "headers": dict(response.headers)}
```

**`miles_admission_disabled` 语义边界 (P0 — 简化代价显式 surface)**:
    ...
    with self._init_lock:
        ...
        try:
            ...
            self.actor_train = RayTrainGroup(
                ...
                # RLix retained-offload path 必须 tiny Ray GPU reservation (P0):
            ...
            # 别的 pipeline 拿不到 GPU. **不做 actor cleanup**: first build 失败语义
            # 是 raise → driver process exit → 用户 shell `ray stop` 收尾 (见 F12 cleanup 段).
        ...
        #   失败路径 (Step 8/9/10 raise) 必须 release, 否则 scheduler 永远占着 GPU
        self._request_cluster_gpus(
        ...
        try:
            ...
            # Step 10: bootstrap coordinator active set
            ...
            # actor_infer init failed → release scheduler. **不做 actor cleanup**: first
            # build 失败语义是 raise → driver process exit → 用户 shell `ray stop` 收一切.
            # actor_infer allocation 是 GENERATION priority long-lived, 成功后由 main
            # loop 的 stop / pipeline destroy 路径 release; 仅 init 失败这里 release.
            # release 用 try/log/swallow 包裹避免掩盖原 init 异常.
            try:
                self._notify_release_cluster_gpus(
                    cluster_id=self._actor_infer_cluster_id,
                    global_step=init_global_step,
...
5. **`MilesModelUpdateService.sync_selected_workers(sync_id, tgt_engine_indices)`** —
   ...
   **关键 strict invariants（受 `_cache_lock` 保护，压短 pseudo-code 时不能损失）**：
   ...
   - **`args.miles_model_update_bucket_size_mb` 新增 arg**（默认 512 MB；不复用
     ...
     peak VRAM 约束）；启动时用"wake_up 后剩余 VRAM"做上界 check，确保
     `bucket_size_bytes + transport scratch < overlap GPU 可用余量`。F10 topology validation
     做这个 check (而不是 lazy 在首次 sync 时报错)
...
| Transport | mask 字段 | colocate? |
|---|---|---|
| `cuda_ipc` (默认) | **`ipc_local_ranks`** | colocate |
| `cpu_serialize` (fallback) | **`cpu_serialize_local_ranks`** | colocate |
| NCCL broadcast | **`broadcast_local_ranks`** | non-colocate |

receiver-side guard:

- `update_weights_from_tensor(serialized_named_tensors, ipc_local_ranks, ...)` (cuda_ipc
  路径): **既有 method 必须扩展 `ipc_local_ranks` 参数**, 检查 `self.rank in ipc_local_ranks`,
  不在则 skip。对齐 ROLL [vllm_strategy.py:685](external/ROLL/roll/distributed/strategy/vllm_strategy.py#L685)
  签名 `update_weights_from_tensor(... ipc_local_ranks=None, model_update_transport="cuda_ipc")`。
  MILES 现状的 [sglang_engine.py:282](external/miles/miles/backends/sglang_utils/sglang_engine.py#L282)
  `update_weights_from_tensor` 没有该参数, **必须改签名加 mask**, 否则 tp>1 时 IPC-only
  ranks 也会被驱动跑 broadcast 接收路径 → 状态错乱
- `update_weights_from_cpu_bucket(payload_bytes, cpu_serialize_local_ranks, ...)` (cpu_serialize
  路径): 检查 `self.rank in cpu_serialize_local_ranks`, 不在则 skip. **`payload_bytes`
  不是 `payload_ref`** — Ray 自动解引用 remote method 的 top-level ObjectRef 参数,
  receiver 实际收到 `bytes`; 内部 `torch.load(BytesIO(payload_bytes), ...)` 不要
  `ray.get(payload_bytes)` (会 raise `TypeError`). Sender 仍 `ray.put(...) → ref`,
  调用 `engine.update_weights_from_cpu_bucket.remote(ref, ...)`, Ray runtime 在
  method 调用边界自动解引用
...
- `destroy_collective_group(group_name)` 必须用 `is_group_exist(group_name)` no-op guard
  ...
  guard 会 KeyError
- 所有 parity gate 都按 tp>1 运行, 必须覆盖 cuda_ipc-only ranks, cpu_serialize-only ranks,
  broadcast ranks 与混合 receiver mask; 不能依赖 tp=1 绕过该 bug。
...
##### Case A：CUDA IPC (first build follow-up, RLix mode 不走)

**First build RLix mode 强制 `args.model_update_transport == "cpu_serialize"`** (F10
已 assert). cuda_ipc adapter 留作 follow-up: 既有
[`_send_to_colocated_engine`](external/miles/miles/backends/megatron_utils/update_weight/update_weight_from_tensor.py#L280)
依赖 live `dist.gather_object(..., group=ipc_gather_group)`
...
NCCL → sync) 顺序不兼容. **不能复用现有 `_send_to_colocated_engine`**, 需要新写
"CPU cache → per-bucket H2D staging → IPC handle serialize" adapter (~150 行).

standalone mode 仍可用 cuda_ipc 既有路径 (无 destroy NCCL 依赖).
...
`FlattenedTensorBucket` 嵌套结构**）：
  ...
  **注意 Ray ObjectRef auto-deref**: Ray 自动解引用 remote method 的 top-level
  `ObjectRef` 参数, 所以 receiver 实际收到的是 `bytes`, 不是 `ObjectRef`. 内部
  **不要 `ray.get(payload_bytes)` (会 raise `TypeError`)**:

  ```python
  # miles/backends/sglang_utils/sglang_engine.py
  def update_weights_from_cpu_bucket(self, payload_bytes, load_format=..., flush_cache=False, weight_version=None):
      # payload_bytes 已被 Ray 自动 deref (top-level ObjectRef → bytes), 不要 ray.get
  ...
  Ray runtime 在 method 调用边界自动解引用.
  ...
  **背景（仅适用 Case B）**：MILES 是首个把 cpu_serialize 引入 SGLang 链路的实现。
  ...
  CAP_SYS_PTRACE / `--ipc=host`）下不可用. **本 milestone RLix mode 强制
  `args.model_update_transport == "cpu_serialize"`** (F10 validation, 见上文): 既有
  `_send_to_colocated_engine` cuda_ipc 路径依赖 live `dist.gather_object` NCCL group
  ([update_weight_from_tensor.py:319](external/miles/miles/backends/megatron_utils/update_weight/update_weight_from_tensor.py#L319))
  + 当前 GPU tensor 打包 IPC handle, 与 F4 sequence (build cache → offload →
  destroy NCCL → sync) 不兼容. cuda_ipc adapter (CPU cache → H2D staging → IPC
  handle serialize, **不能复用 `_send_to_colocated_engine`**) 留作 follow-up.
  standalone mode 仍可走 cuda_ipc Case A.
  ...
  **LoRA 平行处理（仅 Case B）**：cuda_ipc 路径下 `load_lora_adapter_from_tensors`
  ...
  不动；cpu_serialize 路径平行新增 `update_lora_adapter_from_cpu_bucket(payload_bytes,
  lora_name, cpu_serialize_local_ranks, ...)` Ray actor method（命名与
  `update_weights_from_cpu_bucket` 对称, payload_bytes 同 auto-deref 处理）+ SGLang
  vendored patch 对应 dispatch。Wire payload schema 与 weight bucket
...
#### Active set bootstrap (P0 — 否则首次 active refresh 短路)
...
**Hooks 解耦不变量（P0 — 严守 import seam）**: `examples/fully_async/fully_async_rollout.py`
...
- `begin_progress_batch(target_weight_version, step_target_groups)`（**硬不变量**）：
...
# offload_train must be True (P0): MILES MegatronTrainRayActor.sleep() (actor.py:198)
...
# rollout_num_gpus divisibility (P0): floor division 不整除会让 ROLL
...
# RLix mode 强制 cpu_serialize (P0): cuda_ipc 既有路径依赖 live NCCL group
# (dist.gather_object); F4 序列是 build cache → offload → destroy NCCL → sync, 
# destroy 后 cuda_ipc 不能跑. cuda_ipc adapter 留作 follow-up
if DO_TIME_SHARING:
    assert getattr(args, "model_update_transport", "cuda_ipc") == "cpu_serialize", (
        "RLix mode first build forces model_update_transport=cpu_serialize; "
        "cuda_ipc adapter (CPU cache → H2D staging → IPC handle) is follow-up"
    )
...
# Bucket size 不能超过 wake_up 后剩余 VRAM (F4)
...
transport_scratch_bytes = 256 * 1024 * 1024  # NCCL + Ray ObjectRef 经验值
...
2. 新增 `MilesPlacementProvider` adapter（`miles/ray/placement_provider.py`）：
   - **从 coordinator 注入 `RollResourceManagerProxy`, 不自建** — `PipelineCoordinator.__init__`
     ...
     pipeline 转手传给 `MilesPlacementProvider(proxy_handle, args)`。**禁止 provider 自己
     `RollResourceManagerProxy(...)` 实例化** — 会 double-init 并 break shared PG 不变量
   - 调用 `proxy.allocate_placement_group(world_size=..., device_mapping=...)` 获取
     shared PG slot (per role, per pipeline)
   - 从 RLix/ROLL 的 per-worker device mapping 显式推导 `actor_pg / rollout_pg /
     actor_bundle_indices / rollout_bundle_indices / actor_gpu_ids / rollout_gpu_ids`
...
class WorkerPlacement:
    ...
    placement_group: PlacementGroup   # ROLL 节点级 PG 引用
    node_rank: int
    gpu_ids: list[int]                # 该 worker 占的 GPU id list (len = gpus_per_worker)

class MilesPlacementProvider:
    def __init__(self, proxy: "RollResourceManagerProxy", args):
    ...
    def get_rollout_workers(self) -> list[WorkerPlacement]:
        if self._rollout_workers is None:
            world_size = self._args.rollout_num_gpus // self._args.rollout_num_gpus_per_engine
            device_mapping = list(range(self._args.rollout_num_gpus))
            raw = self._proxy.allocate_placement_group(
                world_size=world_size, device_mapping=device_mapping,
            )
    ...
    def get_train_workers(self) -> list[WorkerPlacement]:
        # 同上, world_size = actor_num_nodes * actor_num_gpus_per_node, 每 worker 1 GPU
        if self._train_workers is None:
            world_size = self._args.actor_num_nodes * self._args.actor_num_gpus_per_node
            device_mapping = list(range(world_size))
            raw = self._proxy.allocate_placement_group(
                world_size=world_size, device_mapping=device_mapping,
            )
            self._train_workers = [
...
- **First build 强约束**: `assert args.rollout_num_gpus_per_engine <= args.num_gpus_per_node`
  ——`WorkerPlacement.placement_group` 只指向单个节点 PG, 跨节点 TP engine 当前
  不支持. 多节点 TP 留 follow-up
- **Ray actor resource: standalone 沿用 fractional, RLix 必须 tiny reservation (P0)**:
...
- **手动设 `CUDA_VISIBLE_DEVICES = ",".join(str(g) for g in wp.gpu_ids)` + 必须同时
  ...
  ```
- **`SGLangEngine` 必须显式传 `base_gpu_id=wp.gpu_ids[0]` + `num_gpus_per_engine=len(wp.gpu_ids)`**;
  **不能 fallback 到 `get_base_gpu_id(args, rank)`** ([sglang_engine.py:24-34](external/miles/miles/backends/sglang_utils/sglang_engine.py#L24)) —
  ...
  RLix-derived partial-overlap 下不可靠
...
| `miles/ray/placement_provider.py` (**新增**) | F12 | `MilesPlacementProvider` 真实 adapter — 接收注入的 `RollResourceManagerProxy` (**不自建**, 从 coordinator 传入) + `WorkerPlacement` dataclass (`placement_group / node_rank / gpu_ids`, per-worker view of ROLL `List[List[Dict]]`) + `get_rollout_workers() / get_train_workers()` 调 `proxy.allocate_placement_group(world_size, device_mapping)` 切成 per-worker view + 启动期 structural assert (`len(workers) == 派生`, `len(wp.gpu_ids) == tp`, `wp.gpu_ids == sorted(...)` 验证 first build contiguous; **不做 identity scheduler_to_engine round-trip** — first build 拓扑下永远 pass, 是 dead assert; **不做 `scheduler.get_allocation` cross-check** — RLix 没此 public API; 二者都留 follow-up) | +100 |
| `miles/ray/placement_group.py` | F12 | `create_placement_groups(args, *, external_provider=None)` | +20 |
| `miles/utils/arguments.py` | F1, F4, F10 | 新增 RLix-mode tuning args (3 个 new): `args.miles_model_update_bucket_size_mb` (默认 512), `args.miles_post_sleep_vram_threshold_gb` (默认 1.0), `args.model_update_transport` (默认 `"cuda_ipc"`, RLix mode 由 F10 强制覆盖到 `"cpu_serialize"`). **不新增 device_mapping args** (派生路径). `args.offload_train` 既有, F10 fail-fast 强制 True | +30 |
| `examples/fully_async/fully_async_rollout.py` | F2, F3, F9 | abort 触发口 + 接受 `rlix_hooks` 参数 (standalone 走 `NoOpRLixHooks`) + `begin_progress_batch(target_weight_version, step_target_groups=args.rollout_batch_size)` / `bump_completed(target_weight_version=...)` / `end_progress_batch()` 包裹 wait window (**只调 hook 抽象, 不 import RLix 类型, 不 `ray.get_actor` coordinator**) + **`_FatalError` sentinel + `task_done_callback` catch `(EnginePreemptedError, RLixRouterMetadataError)` 走 queue sentinel + 主循环 dequeue 检测 `isinstance(result, _FatalError)` raise** (queue 单路径已足够, **不引入 `worker._fatal_error` flag**) | +50 |
...
| 文件 | Feature | 改动 | 行数 |
|---|---|---|---|
| `rlix/pipeline/miles_pipeline.py` (**新增**) | F5+6, F8, F10, F11, F12 | `MilesPipeline` actor — registration + validation (F10 含 single_updateable / EP fail-fast / stream / bucket-size / **`assert args.offload_train`** / **`rollout_num_gpus % rollout_num_gpus_per_engine == 0`** / **RLix mode 强制 `model_update_transport == "cpu_serialize"`**) + resize_infer + expand+selective sync + `_after_training` hook + `_init_lock: threading.Lock` 字段 + init bootstrap **sync def + `run(coro)`** (不 async, 避免半 async/sync 阻塞 event loop) **新顺序** (Step 1 request train → Step 1b RayTrainGroup ctor with `worker_placements=` and `num_gpus_per_actor=0.01` → Step 2 `run(actor_train.init())` → Step 3 `run(actor_train.onload())` → Step 4 build cache → Step 5 `run(actor_train.offload())` → finally: try/log/swallow release train (避免 release fail 替换原 init 异常) → Step 7 request infer → Step 8 `RolloutManager.options(...).remote(args, worker_placements=...)` → Step 9 `get_engine_count()` sanity → Step 10 `_get_coordinator_handle().bootstrap_active_engines.remote(...)` → except: try/log/swallow release infer + raise) + `_get_coordinator_handle()` lazy resolver (复用 ROLL pattern) + bundle mapping consume `MilesPlacementProvider.get_train_workers/get_rollout_workers`. **不引入 `actor_train_init_ok` flag + cleanup 分支** + **不引入 `actor_infer.shutdown.remote()` cleanup** (first build 失败 = raise, 见 F12 (c)) | +470 |
| `rlix/pipeline/miles_coordinator.py` (**新增**) | F3, F5+6, F7, F9 | `class MilesCoordinator(Coordinator)` — **不 subclass `PipelineCoordinator`, 不 call super().__init__** (避免触发 ROLL `_validate_config_schema` / `_validate_cpu_only_reward` / `_validate_vllm_sleep_level` / `_validate_offload_nccl` 4 个吃 ROLL config 字段 validator, MILES args 没这些字段). **手动 init**: `_pipeline_id`, `_ray_namespace`, `_pipeline_env_vars`, `_resource_manager_proxy` (singleton, 注入给 placement provider), `_resize_sync_lock`, `_progress_lock`, `_scheduler_reports`, `_coord_progress_last_bucket`, `_active_engine_indices: Set[int]`, `_rlix_scheduler` actor handle, `_model_update_service` lazy slot. **复制并适配** (不继承): `report_progress_from_scheduler` / `clear_progress_stream` / `_aggregate_and_emit` / `_inject_pipeline_env_vars` 4 个 backend-neutral 方法. **必须实现 Coordinator ABC abstract `sync_lora_weights`** (LoRA out of scope, raise unsupported stub, 否则 ABC 实例化 TypeError). **新增**: `bootstrap_active_engines(engine_indices)`, `sync_base_weights_to_active() -> None`, `resize_infer` (first build identity 拓扑, scheduler dp_rank == MILES engine_index, 直接调 `RolloutManager.shrink_engines/expand_engines`; 非连续 mapping 解禁时引入 `MilesPlacementProvider.scheduler_to_engine` 双向 lookup, follow-up), `create_pipeline_actor(*, pipeline_config)` 创建 `MilesPipeline` actor | +280 |
| `rlix/pipeline/miles_model_update_service.py` (**新增**) | F4, F5+6 | 简化版 ModelUpdateService (selective sync 双 colocate transport: cuda_ipc 默认 + receiver mask = `ipc_local_ranks`, cpu_serialize fallback + receiver mask = `cpu_serialize_local_ranks`, **RLix mode 强制 cpu_serialize**, cuda_ipc 路径 follow-up 因为依赖 live NCCL group) + 动态 NCCL group 生命周期 + 单槽 versioning + warmup allreduce + port claim 释放 (happy path 无条件回 pool; 不处理 receiver crash 容错) + **cache owner 由 init 阶段 worker `report_cache_owner_role.remote()` 上报, 不在 sync 路径运行时查询** + `sync_id` 参数 + init bootstrap step=-1 path + 复用 RLix `ROLL_SELECTIVE_MODEL_UPDATE_TIMEOUT_S` env var (**不再造 MILES_***) | +290 |
...
**总计：~2455 行** (在 "~2790 production-grade" 基础上经第四轮简化 pass 砍掉 ~335
行防御性工程):

砍掉 (first build 不必要):
- Cleanup APIs (RolloutManager.shutdown + RayTrainGroup.shutdown + __init__ self-cleanup
  + force-kill timeout fallback): -150
- F4 init bootstrap actor_train_init_ok flag + finally cleanup 分支 + actor_infer
  shutdown.remote + release double-log: -70
- F3 router admission_epoch race 防御 + epoch start/end 比对: -50
...
**失败语义统一**: first build 失败 = raise → driver process exit → 用户 shell
`ray stop && ray start` 收尾 (CLI 杀 node 上所有 Ray 进程, 无视 actor lifetime).
不引入 driver 顶层 `ray.shutdown()`, 不约束 actor `lifetime`. multi-pipeline cleanup
race (Gate 4 之后) 与 receiver crash 容错留作 production hardening follow-up.
...
- **First build 失败语义 = raise → driver process exit → 用户 shell `ray stop && ray
  start ...` 收尾**. CLI `ray stop` 杀 node 上所有 Ray 进程 (raylet / GCS / actor /
  SGLang child / PG), 无视 actor lifetime 与 driver 退出方式 (ctrl-C / SIGTERM /
  uncaught exception 都 OK). 不需要 driver 顶层 try/except + `ray.shutdown()`, 也不
  约束 actor `lifetime`. multi-pipeline cleanup race (Gate 4 之后) 与 receiver
  crash 容错留 follow-up
- **Multi-node TP engine 当前不支持** (F12 first build 强约束
  `rollout_num_gpus_per_engine <= num_gpus_per_node`). 多节点 TP follow-up:
  扩 `WorkerPlacement` 成 per-node placement list, 按 node-rank 分别建 SGLang sub-actor
- **cuda_ipc selective sync 路径当前 follow-up** — 既有 `_send_to_colocated_engine`
  依赖 live `dist.gather_object` NCCL group, 与 F4 destroy NCCL 顺序不兼容. F10 RLix
  mode 强制 `model_update_transport == "cpu_serialize"`. cuda_ipc adapter (CPU cache
  → H2D staging → IPC handle) 留作 follow-up (~150 行)
...
| cuda_ipc selective sync adapter (CPU cache → per-bucket H2D staging → IPC handle serialize, **不复用 `_send_to_colocated_engine`**) | 当 cpu_serialize transport 成为吞吐瓶颈; ~150 行 |
| router `admission_epoch` race 防御 (start/end epoch 比对消除 disable→abort→enable race) | production 多 pipeline 高频 shrink/expand 触发 false negative; first build turn retry 兜底, race window ms 级实际拓扑不发生 |
