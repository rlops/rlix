...
`NemoRLRLixHooks.report_progress` 构造 `ProgressReport` 并 fire-and-forget 发给 coordinator。首个 batch-begin 快照带 `new_batch=True`，后续 bucket 更新带 `new_batch=False`，与 ROLL 对齐。`clear_progress` 调用 `coordinator.clear_progress_stream(mode, adapter_id)`（`coordinator.py:326`）— 注意 API 是 `clear_progress_stream` 不是 `clear_progress`，coordinator 聚合层负责判断是否还有其他 active streams 并决定是否通知 scheduler。
